...
This example sets `standoff` attribute to cartesian axes to determine the distance between the tick labels and the axis title. Note that the axis title position is always constrained within the margins, so the actual standoff distance is always less than the set or default value. By setting standoff and turning [automargin](https://plot.ly/python/setting-graph-size/#automatically-adjust-margins) on, plotly.js will push the margins to fit the axis title at given standoff distance.
...
### Share color axis
This example shows how to set a reference (coloraxis, coloraxis2, coloraxis3) to a shared [color axis](https://plot.ly/javascript/reference/#scatter-marker-line-coloraxis), which are set in the layout. Note that multiple color scales can be linked to the same color.

```python
import plotly.graph_objects as go

fig = go.Figure(go.Heatmap(
    name = "heatmap1, coloraxis1",
    x = [1, 2],
    z = [[1, 2], [4, -3]],
    coloraxis = "coloraxis"))

fig.add_trace(go.Heatmap(
    name = "heatmap2, coloraxis1",
    x = [3, 4],
    z = [[10, 2], [4, 3]],
    coloraxis = "coloraxis"))

fig.add_trace(go.Heatmap(
    name = "heatmap3, coloraxis1",
    x = [5, 6],
    z = [[3, -2], [4, 5]],
    coloraxis = "coloraxis"))

fig.add_trace(go.Bar(
    name = "bar marker, coloraxis2",
    y = [1, 2, 1],
    marker = {
      "color": [20, 2, 30],
